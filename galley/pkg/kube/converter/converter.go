...
var converters = map[string]Fn{
	...
	"oidc-policy-resource":  oidcPolicyResource,
}
...
func oidcPolicyResource(destination resource.Info, name string, u *unstructured.Unstructured) (string, time.Time, proto.Message, error) {
	p, err := toProto(destination, u.Object["spec"])
	if err != nil {
		return "", time.Time{}, nil, err
	}
