...
var knownTypes = map[string]schemaType{
	...
	model.OidcPolicy.Type: {
		schema: model.OidcPolicy,
		object: &OidcPolicy{
			TypeMeta: meta_v1.TypeMeta{
				Kind:       "OidcPolicy",
				APIVersion: apiVersion(&model.OidcPolicy),
			},
		},
		collection: &OidcPolicyList{},
	},
}
...
// OidcPolicy is the generic Kubernetes API object wrapper
...
// GetSpec from a wrapper
func (in *OidcPolicy) GetSpec() map[string]interface{} {
	return in.Spec
}

// SetSpec for a wrapper
func (in *OidcPolicy) SetSpec(spec map[string]interface{}) {
	in.Spec = spec
}

// GetObjectMeta from a wrapper
func (in *OidcPolicy) GetObjectMeta() meta_v1.ObjectMeta {
	return in.ObjectMeta
}

// SetObjectMeta for a wrapper
func (in *OidcPolicy) SetObjectMeta(metadata meta_v1.ObjectMeta) {
	in.ObjectMeta = metadata
}

// OidcPolicyList is the generic Kubernetes API list wrapper
...
// GetItems from a wrapper
func (in *OidcPolicyList) GetItems() []IstioObject {
	out := make([]IstioObject, len(in.Items))
	for i := range in.Items {
		out[i] = &in.Items[i]
	}
